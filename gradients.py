"""
Allow to paint some gradients relatively easy.
...
class ColorInterpolator(object):
    ...
    def evaluate(self, x):
        ...
        '''
        return [self.rInterpolator.evaluate(x),
                self.gInterpolator.evaluate(x),
                self.bInterpolator.evaluate(x),
                self.aInterpolator.evaluate(x)]
...
def radial(size, startcolor, endcolor):
    """
    Draws a linear radial gradient on a rectangular sized surface and returns
    that surface.
    """
    width = size[0]
    height = size[1]
    bigSurf = pygame.Surface((width, height)).convert_alpha()
    bigSurf.fill(endcolor, (0, 0, width, height))
    ...
    for rad in range(radius, 0, -1):
        draw_circle(bigSurf, (er + int(rm * rad),
                              eg + int(gm * rad),
                              eb + int(bm * rad),
                              ea + int(am * rad)), (width // 2, height // 2), rad)
    return bigSurf

def squared(width, startcolor, endcolor):
    """
    Draws a linear squared gradient on a square sized surface and returns
    that surface.
    """
...
def vertical_func(size, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1)):
    ...
    for y in range(0, height):
        bigSurf.set_at((0, y), color.evaluate(y + 0.1))
    return pygame.transform.scale(bigSurf, size)
...
def horizontal_func(size, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1)):
    ...
    for y in range(0, width):
        bigSurf.set_at((y, 0), color.evaluate(y + 0.1))
    return pygame.transform.scale(bigSurf, size)
...
def radial_func(radius, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1), colorkey=(0, 0, 0, 0)):
    """
    Draws a linear radial gradient on a square sized surface and returns
    that surface.
    """
    ...
    draw_circle = pygame.draw.circle
    for rad in range(radius, 0, -1):
        draw_circle(bigSurf, color.evaluate(rad), (radius, radius), rad)
    return bigSurf
...
def radial_func_offset(radius, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1), colorkey=(0, 0, 0, 0), offset=(0, 0)):
    ...
    Unfortunately this function ignores alpha.
    """
    bigSurf = pygame.Surface((2 * radius, 2 * radius))#.convert_alpha()
    ...
    for rad in range(radi, 0, -1):
        draw_circle(bigSurf, color.evaluate(rad), (radius + offset[0], radius + offset[1]), rad)
...
def squared_func(width, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1), offset=(0, 0)):
    ...
    for currentw in range((widthh // 2), 0, -1):
        ...
        draw_rect(bigSurf, color.evaluate(currentw), rect)
    return bigSurf
...
def draw_gradient(surface, startpoint, endpoint, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1), mode=0):
    """
    Instead of returning an Surface, this function paint it directly onto the 
...
def draw_circle(surface, startpoint, endpoint, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1), mode=0):
    """
    Instead of returning an Surface, this function paint it directy onto the 
    given Surface and returns the rect.
    """
    dx = endpoint[0] - startpoint[0]
    dy = endpoint[1] - startpoint[1]
...
def chart(startpoint, endpoint, startcolor, endcolor, Rfunc=(lambda x:x), Gfunc=(lambda x:x), Bfunc=(lambda x:x), Afunc=(lambda x:1), scale=None):
    ...
    (the width of the image) is shown as a line.
    ...
    bigSurf.fill((0,) * 3)
    oldcol = color.evaluate(0)
    for x in range(distance):
        r, g, b, a = color.evaluate(x)
...
def genericFxyGradient(surf, clip, color1, color2, func, intx, yint, zint=None):
    ...
    surf   : surface to paint
    clip   : rect on surf to paint in
    color1 : start color
    color2 : end color
    ...
    # calculate stretching and displacement variables
    a = ((color2[0] - color1[0]) / (z2 - z1), \
         (color2[1] - color1[1]) / (z2 - z1), \
         (color2[2] - color1[2]) / (z2 - z1), \
         (color2[3] - color1[3]) / (z2 - z1)) # stretching in z direction
    b = (x2 - x1) / float(w) # stretching in x direction
    d = (y2 - y1) / float(h) # stretching in y direction
    f = (color1[0] - a[0]*z1, \
          color1[1] - a[1]*z1, \
          color1[2] - a[2]*z1, \
